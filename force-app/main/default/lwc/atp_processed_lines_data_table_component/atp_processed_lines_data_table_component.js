...
//apex method to publish resubmit platform event
...
export default class Atp_processed_lines_datatable_component extends NavigationMixin(LightningElement) {
    ...
    sortDirection = 'asc';
    //variable current sorted by column
    ...
    //variable for fields to be query
    fieldsToQuery;
    //variable to store the selected Fields
    selectedFieldsForSearch = [];
    //variable for earch key entered by User
    searchKey;
    ...
    getColumns({ error, data }) {
        if (data) {
            ...
            if (this.fieldsToQuery) {
                this.dispatchEvent(new CustomEvent('queryprocessedlinerecordsevent'));
            }
        } else if (error) {
           // console.log(error);
        }
    ...
    /**
        * to get fetch atp lines records and its count
    ...
    /**
      * to preparee the SOQL Query based on user selected filters
      */
    prepareSOQLQuery(isCountQuery) {
        ...
        let whereClause = `SVMXC__Parts_Request__c = '${this.partsRequestId}' AND Requested_Part__c IN (${this.productIdsString}) AND RecordTypeId='${this.partsReqLineAtpCheckLineRecordTypeId}' AND SVMXC__Line_Status__c!='Open'  `;
    ...
    */
    ...
    /**
      * to refresh processed atp line records
      */
    @api handleRefresh(sourceValues) {
        this.searchKey = '';
        this.selectedSourceValues = sourceValues;
        this.getPartsReqLineCount();
        this.getActualLinesData();
        this.errors = null;
        this.errorRowIds=[]
    ...
    /**
      * to handle sort column is clicked by user
    ...
    /**
       ...
       * this will get the atp lines data as per new page size
        */
